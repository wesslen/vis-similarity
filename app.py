...
def main():
	...
	approach = st.sidebar.selectbox('',['Embedding','Keyword'])

	if approach == "Embedding":

		st.sidebar.markdown('**Query Paper**')
		option = st.sidebar.multiselect('Selecting multiple will average the query vectors:',papers) # index="The Anchoring Effect in Decision-Making with Visual Analytics"
		...
		if True: # need to remove
			c = plot_scatter(df, query = option, highlight = results.head(n=select_num), color = color_option,  brush_option = brush_option)
		...
		select_keyword = st.sidebar.multiselect('Select Keywords:',keywords['Keyword'])

		#keyword_papers = df.index[df['Abstract'].str.match(select_keyword.value, case = False)]

		st.table(select_keyword)
...
def plot_scatter(df, query = None, highlight = None, color = "Similarity", brush_option = False):
	...
	if color == "Similarity":
		points = alt.Chart(df).mark_circle(size=size, opacity=alpha).encode(
		    x='X',
		    y='Y',
		    color = alt.Color('Highlight:N', scale=alt.Scale(domain=domain, range=range_), legend=None), #remove legend
